...
- UDMS의 `targetType` 확장은 bootstrap 기반 `Dynamic Target Registry`와 `/api/v1/udms/target-types` catalog를 기준으로 관리한다.
...
- dashboard 기본 화면
- UDMS 모듈별 페이지
- UDMS target catalog 기반 selector 및 metadata 렌더링
- 현재 UDMS write-enabled target은 `Board`만 활성화
- 예배 순서 / 자막 입력 / 자막 출력
- 관리자 사용자 관리
- display 전용 화면
...
- 인증 사용자는 공통 글로벌 헤더를 공유한다.
- 글로벌 헤더는 `Dashboard`, `UDMS`, `Worship`, `Admin` 모듈 전환만 담당한다.
- `Admin`은 `master`에게만 노출한다.

### 5.2 좌측 컨텍스트 내비게이션

- 좌측 내비게이션은 현재 활성 모듈의 메뉴만 보여준다.
- 메뉴는 `그룹 > 하위 메뉴`의 2단 구조로 고정한다.
- 그룹은 아코디언으로 동작하고, 현재 경로가 속한 그룹이 열린다.
