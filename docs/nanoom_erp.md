...
레이아웃 정보구조의 상세 기준은 [layout.md](layout.md)를 따른다.
...
- 상단 `Global`과 좌측 `Contextual`을 분리해 인지 부하를 낮춘다.
- 공개 영역, 인증 워크스페이스, 관리자, 송출 화면은 서로 다른 레이아웃 규칙을 사용한다.
...
6. 공통 레이아웃 셸과 `/dashboard` 진입점을 정리한다.
...
- `/`는 공개 랜딩이다.
- `/login`은 인증 시작점이다.
- 공개 영역은 읽기 중심 콘텐츠와 로그인 동선만 가진다.

### 8.2 인증 워크스페이스

- `/dashboard`는 로그인 후 일반 사용자의 기본 진입점이다.
- `/udms/*`는 문서 업무를 담당한다.
- `/worship/*`는 예배 업무를 담당한다.

### 8.3 관리자 영역

- `/admin/*`은 운영과 설정을 담당하는 관리자 모듈이다.
- 관리자도 같은 인증 체계를 공유하지만, 일반 업무 공간과는 분리된 정보구조를 사용한다.

### 8.4 송출 화면

- `/display`는 전체화면 송출 UX를 유지하는 예외 화면이다.
- 공통 헤더나 사이드바를 얹지 않는다.
