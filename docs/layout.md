...
- `/`는 공개 랜딩이고, 인증 사용자는 `/dashboard` 또는 `/admin`으로 이동한다.
- `(workspace)`와 `(admin)`은 공통 `AuthenticatedShell`을 사용한다.
- 글로벌 헤더에서 역할별 모듈 스위처를 제공한다.
- 좌측 사이드바는 현재 활성 모듈의 그룹과 하위 메뉴만 2단 구조로 보여준다.
- 모바일은 `모듈` 버튼과 `메뉴` 버튼을 분리해 module switcher와 contextual drawer를 따로 연다.
- `/display`는 셸 밖의 독립 화면으로 유지한다.

## 11. 테스트 결과 요약

아래 결과는 현재 코드, `npm.cmd run typecheck`, `npm.cmd run build` 기준으로 확인한 결과다.

| 테스트 관점 | 기대 동작 | 실제 결과 | 검증 근거 |
| --- | --- | --- | --- |
| 역할 가시성 | `member/editor/final_approver`는 `Dashboard/UDMS/Worship`만, `master`는 `Admin`까지 본다 | 통과 | `AuthenticatedShell`의 `getVisibleModules`가 module 단위 role 필터링을 수행하고, build/typecheck 통과 |
| 인터랙션 | 헤더는 모듈 선택만 담당하고, 좌측은 현재 모듈의 2단 submenu만 보여준다 | 통과 | `activeModule` 계산 후 `activeModule.groups`만 렌더링하고, 헤더 모듈 스위처는 `defaultHref`로만 이동 |
| 모바일 | `모듈` 버튼과 `메뉴` 버튼이 분리되고, route 변경 시 panel이 닫힌다 | 통과 | `mobileModulesOpen`, `mobileMenuOpen`을 분리했고 `pathname` 변경 시 둘 다 닫도록 구현 |
| 예외 경로 | `/display`, `/`, `/login`, `/onboarding`, `/pending`, `/blocked`는 shell 밖에 있어야 한다 | 통과 | shell은 `(workspace)`와 `(admin)` layout에서만 사용되고, build 결과에서 `/display`와 public/attention routes가 별도 경로로 유지됨 |
...
- [x] `/dashboard` 라우트와 사용자 대시보드 페이지 구현
- [x] 공통 authenticated shell 레이아웃 구현
- [x] 글로벌 헤더 모듈 스위처 UI 구현
- [x] 모듈별 contextual sidebar UI 구현
- [x] 모바일 drawer 상호작용 구현
- [x] `/display` 셸 분리 상태 검수
- [x] 구현 후 문서-코드 정합성 검수
